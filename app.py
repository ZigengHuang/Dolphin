...
def chat():
    ...
    if dialogue_type == "dolphin":
        ...
        a1 = call_chatanywhere(f"背景总结: {summary}，患者内容: {message}", context=context_a)
...
def generate_response():
    ...
    if feedback == 'like':
        if dialogue_type == "dolphin":
            ...
            messages.append({"role": "user", "content": f"背景总结: {summary};患者说话内容: {original_message};RAG知识: {rag_knowledge_str}"})
        ...
        context_a = [{
            ...
            "content": """
                ...
                **请注意，你刚刚已经针对这个任务生成了一个输出，但该输出的效果不好，请你进一步思考，尝试不同的输出内容再试一遍
            """
        }]
