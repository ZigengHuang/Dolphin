...
class ApplicationGUI:
    ...
    def create_path_controls(self):
        self.create_label_entry("RAG model path:", 0)
        self.create_label_entry("RAG knowledge path:", 1)
        self.create_label_entry("audio model path:", 2)  # 新增的输入框
        self.create_label_entry("audio save path:", 3)
        self.create_label_entry("data save path:", 4)
    ...
    def on_browse_click(self, entry, label_text):
        if label_text == "RAG model path:":
            folder_path = filedialog.askdirectory()
            entry.delete(0, tk.END)
            entry.insert(0, folder_path)
            self.rag_model_path = folder_path
        elif label_text == "RAG knowledge path:":
            file_path = filedialog.askopenfilename()
            entry.delete(0, tk.END)
            entry.insert(0, file_path)
            self.rag_knowledge_path = file_path
        elif label_text == "audio model path:":  # 新增的处理
            folder_path = filedialog.askdirectory()
            entry.delete(0, tk.END)
            entry.insert(0, folder_path)
            self.audio_model_path = folder_path
        elif label_text == "audio save path:":
            folder_path = filedialog.askdirectory()
            entry.delete(0, tk.END)
            entry.insert(0, folder_path)
            self.audio_save_path = folder_path
        elif label_text == "data save path:":
            folder_path = filedialog.askdirectory()
            entry.delete(0, tk.END)
            entry.insert(0, folder_path)
    ...
    def start_server(self):
        if self.process and self.process.poll() is None:
            messagebox.showwarning("Prompt:", "app.py has been running")
            return
        ...
        if not all([self.rag_model_path, self.rag_knowledge_path, self.audio_model_path, self.audio_save_path, self.db_path, self.api_key]):
            messagebox.showerror("error", "Please fill in all paths and API keys!")
            return
